...
//#pragma omp parallel for private(i) default(none) shared(T,active,nact,p_m,p_t,r00,r01,r02,r10,r11,r12,r20,r21,r22,t0,t1,t2) reduction(+:mum0,mum1,mum2, mut0,mut1,mut2) // schedule (dynamic,2)
  ...
  Matrix q_m = p_m - Matrix::ones((int32_t)active.size(),1)*mu_m;
  Matrix q_t = p_t - Matrix::ones((int32_t)active.size(),1)*mu_t;
...
double IcpPointToPoint::getICPError(double* T, const int32_t T_num, Matrix& R, Matrix& t, const double indist)
{
    int i;
    vector<int32_t> active = getInliers(T, T_num, R, t, indist);  // 특정 거리 안의 점들만 추출
    int nact = (int)active.size();
    double dist_sum = 0.;
    ...
    // establish correspondences
#pragma omp parallel for private(i) default(none) shared(T,active,nact,r00,r01,r02,r10,r11,r12,r20,r21,r22,t0,t1,t2) reduction(+:dist_sum) // schedule (dynamic,2)
    for (i = 0; i < nact; i++) 
    {
        // kd tree query + result
        std::vector<float>         query(dim);
        ...
        // search nearest neighbor
        M_tree->n_nearest(query, 1, result);
        dist_sum += result[0].dis;
    }
    std::cout << nact << std::endl;
...
{
    // init inlier vector + query point + query result
    vector<int32_t>            targetNearestPointsIdx;
    std::vector<float>         query(dim);
    kdtree::KDTreeResultVector neighbor;
    ...
    for (int32_t i = 0; i < T_num; i++) {
        ...
        // search nearest neighbor
        M_tree->n_nearest(query, 1, neighbor);
        targetNearestPointsIdx.push_back(neighbor[0].idx);
    }

    // source의 각 점에서 가장 가까운 target의 점 return
    return targetNearestPointsIdx;
}
